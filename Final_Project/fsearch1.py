...
if frm.getvalue('ok'):
    ...
    cursor.execute("SELECT r.*, p.* FROM wreg r INNER JOIN wpic p ON r.id=p.fid WHERE r.name='{}'".format(name))
    rs=cursor.fetchall()
    if rs:
        ...
        for rec in rs:
            ...
            <td align="center" rowspan=4 width=15%><a href="fcalc1.py?id={}">Sell Here</a></td>
